...
class ongoingTaskRabbitMqEtlEditModel extends ongoingTaskEditModel {
    
    connectionStringName = ko.observable<string>();
        
    allowEtlOnNonEncryptedChannel = ko.observable<boolean>(false);
    transformationScripts = ko.observableArray<ongoingTaskRabbitMqEtlTransformationModel>([]);

    showEditTransformationArea: KnockoutComputed<boolean>;

    transformationScriptSelectedForEdit = ko.observable<ongoingTaskRabbitMqEtlTransformationModel>();
    editedTransformationScriptSandbox = ko.observable<ongoingTaskRabbitMqEtlTransformationModel>(); // make queue // common.. // todo
    ...
    update(dto: Raven.Client.Documents.Operations.OngoingTasks.OngoingTaskQueueEtlDetails) {
        ...
        if (dto.Configuration) {
            ...
            this.transformationScripts(dto.Configuration.Transforms.map(x => new ongoingTaskRabbitMqEtlTransformationModel(x, false, false)));
    ...
    deleteTransformationScript(transformationScript: ongoingTaskRabbitMqEtlTransformationModel) { 
    ...
    editTransformationScript(transformationScript: ongoingTaskRabbitMqEtlTransformationModel) {
        this.transformationScriptSelectedForEdit(transformationScript);
        this.editedTransformationScriptSandbox(new ongoingTaskRabbitMqEtlTransformationModel(transformationScript.toDto(), false, transformationScript.resetScript()));
    }
