...
class ongoingTaskRabbitMqEtlTransformationModel {
    ...
    static isApplyToAll(colectionName: string){
        return colectionName === ongoingTaskRabbitMqEtlTransformationModel.applyToAllCollectionsText;
    }
    ...
    static empty(name?: string): ongoingTaskRabbitMqEtlTransformationModel {
        return new ongoingTaskRabbitMqEtlTransformationModel(
            {
    ...
    addWithBlink(collectionName: string) {
        if (ongoingTaskRabbitMqEtlTransformationModel.isApplyToAll(collectionName)) {
            this.applyScriptForAllCollections(true);
            this.transformScriptCollections([ongoingTaskRabbitMqEtlTransformationModel.applyToAllCollectionsText]);
        } else {
            this.applyScriptForAllCollections(false);
            _.remove(this.transformScriptCollections(), x => x === ongoingTaskRabbitMqEtlTransformationModel.applyToAllCollectionsText);
...
export = ongoingTaskRabbitMqEtlTransformationModel;
