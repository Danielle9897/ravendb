...
class ongoingTaskKafkaEtlTransformationModel {
    ...
    static isApplyToAll(colectionName: string){
        return colectionName === ongoingTaskKafkaEtlTransformationModel.applyToAllCollectionsText;
    }
    ...
    static empty(name?: string): ongoingTaskKafkaEtlTransformationModel {
        return new ongoingTaskKafkaEtlTransformationModel(
            {
    ...
    addWithBlink(collectionName: string) {
        if (ongoingTaskKafkaEtlTransformationModel.isApplyToAll(collectionName)) {
            this.applyScriptForAllCollections(true);
            this.transformScriptCollections([ongoingTaskKafkaEtlTransformationModel.applyToAllCollectionsText]);
        } else {
            this.applyScriptForAllCollections(false);
            _.remove(this.transformScriptCollections(), x => x === ongoingTaskKafkaEtlTransformationModel.applyToAllCollectionsText);
...
export = ongoingTaskKafkaEtlTransformationModel;
