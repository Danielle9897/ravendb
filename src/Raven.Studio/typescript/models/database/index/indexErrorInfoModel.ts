...
class indexErrorInfoModel {
    ...
    //private erroredIndexNames = ko.observableArray<indexNameAndCount>([]); // do i need this here ?
    selectedIndexNames = ko.observableArray<string>([]); // here ?
    ...
    private fetchRemoteIndexesError(): JQueryPromise<IndexErrorPerDocument[]> {
        ...
        return new getIndexesErrorCommand(this.db(), this.nodeTag(), this.shardNumber())
            ...
            .then((result: Raven.Client.Documents.Indexes.IndexErrors[]) => {
                ...
                const indexNamesAndCount = indexErrorInfoModel.extractIndexNamesAndCount(result);
                //const actionNamesAndCount = this.extractActionNamesAndCount(result);

                //this.erroredIndexNames(indexNamesAndCount);
                ...
                //indexErrors.syncMultiSelect(); // TODO in parent..
    ...
    private filterItems(list: IndexErrorPerDocument[]): JQueryPromise<pagedResult<IndexErrorPerDocument>> {
        ...
        // if (this.selectedIndexNames().length !== this.erroredIndexNames().length) {
        //     filteredItems = filteredItems.filter(error => _.includes(this.selectedIndexNames(), error.IndexName));
        // }
    ...
    refresh() {
        if (this.showDetails()) {
            this.indexErrors = null;
            this.gridController().reset(); // todo - why twice ?
        }
    }
}
