...
class nodeStatsItem {
    ...
    disconnected = ko.observable<boolean>(true); // ??? not connected to given node.. !!  todo.. check
    hasData = ko.observable<boolean>(false);

    noData: boolean; // needed for the node tag col.. ? //  todo ?? has data vs no data ???
    database: string; // needed for the node tag col.. ?
...
class clusterOverviewWidget extends websocketBasedWidget<Raven.Server.Dashboard.Cluster.Notifications.ClusterOverviewPayload> {
    ...
    compositionComplete() {
        ...
        this.columnPreview.install(".cluster-grid", ".js-time-preview",
    ...
    private nodeStateDataForHtml(state: Raven.Client.ServerWide.RachisState): iconPlusText[] {
        ...
        switch (state) {
            ...
            // todo - ask Rafal for icons ? Marcin ?
            case "Passive":
            case "Candidate":
            case "Follower":
            case "LeaderElect":
    ...
    onClientDisconnected(ws: clusterDashboardWebSocketClient) {
        ...
        this.withStats(ws.nodeTag, x => x.disconnected(true));  // ??? todo
