...
class indexErrors extends shardViewModelBase {
    ...
    private fetchErrorsDetails(item: indexErrorInfoModel) { // todo remove thens....
        new getIndexesErrorCommand(this.db, item.location())
            .execute()
            .then((resultDto: Raven.Client.Documents.Indexes.IndexErrors[]) => this.mapItems(resultDto))
            .then((result: IndexErrorPerDocument[]) => this.filterItems(result))
            .then((filteredResults: IndexErrorPerDocument[]) => {
