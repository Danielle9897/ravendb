...
class spatialQueryMap extends viewModelBase {
    ...
    createMap() {
        ...
        this.geoData().forEach((layerModel) => {
            ...
            const generatePopupContent = (doc: document) => {
                const docDto = doc.toDto(true); // todo - try run this with false !!!
            ...
            layerModel.geoPoints().forEach((point) => {
                const pointMarker = L.marker([point.latitude, point.longitude], { title: point.popupContent.getId() }) // todo - check the title...
        ...
        let myMap = L.map('mapid', {preferCanvas: true});      // init map
        osmMap.addTo(myMap);                                   // init base layer to use
        L.control.layers(baseLayers, dataLayers).addTo(myMap); // init legend control
        markersGroups.forEach(group => myMap.addLayer(group)); // init markers groups
    ...
    //     let overlaysObj = {};
    //
    //     let clusterGroup1 = L.markerClusterGroup();
    //
    //     this.geoData().forEach((fieldModel) => {
    //         let markersArray: Layer[] = [];
    //
    //         // // // firefox popup content..
    //         // const customPopup = "Mozilla Toronto Offices<br/><img src='http://joshuafrazier.info/images/maptime.gif' alt='maptime logo gif' width='350px'/>";
    //         //
    //         // // firefox specify popup options 
    //         // const customOptions =
    //         //     {
    //         //         'maxWidth': '900',
    //         //         'className' : 'custom'
    //         //     };
    //        
    //         const aaa = (e: any, content: document) => {
    //             const docDto = content.toDto(true);
    //             const metaDto = docDto["@metadata"];
    //             documentMetadata.filterMetadata(metaDto);
    //             const text = JSON.stringify(docDto, null, 4);
    //             app.showBootstrapDialog(new showDataDialog("Document: " + content.getId(), text, "javascript"));
    //         };
    //        
    //         const bbb = (doc: document) => {
    ...
    //                                 <pre>${text}</pre>
    //                              </div>`;
    //             const textHtml2 = `<h4>Document: ${doc.getId()}</h4>
    //                                <pre>${text}</pre>`;
    //             return textHtml;
    //         }
    //        
    //         fieldModel.geoPoints().forEach((point) => {
    //             const pointMarker = L.marker([point.latitude, point.longitude], { title: "show the document id...todo..."})
    //                 .bindPopup(bbb(point.popupContent), { 'className' : 'custom-popup', 'maxWidth': 600, 'maxHeight': 400 } );
    //                 // .bindPopup(customPopup, (customOptions as any) as PopupOptions) // for firefox sample
    //                 // .on('click', (e) => aaa(e, point.popupContent));
    ...
    //         // });
    //
    //         clusterGroup1.addLayers(markersArray);
    //         (overlaysObj as any)[fieldModel.geoField().parentPropertyName] = clusterGroup1;
    //     })
    //
    //     let clusterGroup2 = L.markerClusterGroup();
    //     let markersArray2: Layer[] = [];
    //
    //     for(let i = 0; i< this.addressPoints1.length; i++) {
    //         const testMarker = L.marker([this.addressPoints1[i][0] as number, this.addressPoints1[i][1] as number], { title: "toooltip ****"});
    //         markersArray2.push(testMarker);
    //     }
    //
    //     clusterGroup2.addLayers(markersArray2);
    //     (overlaysObj as any)["sample data"] = clusterGroup2;
    //
    //     ///////////////////////
    //     // define the map
    //     ///////////////////////
    //
    //     // decide the central position for the view first... and initial zoom - the fitBounds overrides this later ....
    ...
    //     L.control.layers(baseLayers, overlaysObj).addTo(myMap); // add to the legend control
    //
    //     //layerGroups.forEach(x => (x).addTo(myMap));
    //     myMap.addLayer(clusterGroup1);
    //     myMap.addLayer(clusterGroup2);
    //
    //     // let allPoints: any[] = [];
    ...
    //     // })
    //
    //     const circle = L.circle([47.624329, -122.3351939], {
    //         color: '#ea647f',
    //         fillColor: '#ea647f',
    //         fillOpacity: 0.4,
    //         radius: 600
    ...
    //     myMap.fitBounds(clusterGroup1.getBounds().extend(clusterGroup2.getBounds()), { padding: [50, 50] });
    // }
    ...
    //                                  .bindPopup(point.tooltipContent);
    //             markersArray.push(pointMarker);                
    //             //Cg.addLayer(pointMarker);
    //         });
    //
    //         Cg1.addLayers(markersArray);
    //
    //         // const Lg = L.layerGroup(markersArray)
    //         // const Fg = L.featureGroup(markersArray)            
    //         //layerGroups.push(Cg);
    //         (overlaysObj as any)[fieldModel.geoField().parentPropertyName] = Cg1;
    //     })
    //
    //     let Cg2 = L.markerClusterGroup();
    //     let markersArray2: Layer[] = [];
    //
    //     // this.addressPoints2.forEach(x => {
    //     //     const testMarker = L.marker([47.614329, -122.32519395]);
    //     //     const testMarker2 = L.marker([47.624329, -122.33519395]);
    //     //     markersArray2.push(testMarker);
    //     //     markersArray2.push(testMarker2);
    //     // })
    //
    //     for(let i = 0; i< this.addressPoints1.length; i++) {
    //         const testMarker = L.marker([this.addressPoints1[i][0] as number, this.addressPoints1[i][1] as number], { title: "toooltip ****"});
    ...
    //     }
    //
    //     Cg2.addLayers(markersArray2);
    //     (overlaysObj as any)["sample data"] = Cg2;
    //
    //     ///////////////////////
    //     // define the map
    //     ///////////////////////
    ...
    //    
    //     //myMap.fitBounds(allPoints, {padding: [50, 50]});
    //     myMap.fitBounds(Cg1.getBounds().extend(Cg2.getBounds()), { padding: [50, 50] });
    // }
