...
class ongoingTasks extends viewModelBase {
    ...
    private initObservables() {
        ...
        // this.taskNameToCount = ko.pureComputed<Record<StudioTaskType, number>>(() => {
        //     return {
        //         "External Replication": this.replicationTasks().length, // todo...
    ...
    attached() {
        ...
        //this.updateUrl(appUrl.forOngoingTasks(db)); // todo check ??? marcin removed in the past
    ...
    private processTasksResult(result: Raven.Server.Web.System.OngoingTasksResult) {
        ...
        // const filteredTaskTypes = this.tasksTypesOrderForUI.filter(x => _.includes(taskTypes, x));
        // const mappedTaskTypes = filteredTaskTypes.map(x => ongoingTaskModel.mapTaskType(x, "Kafka")); // todo - how to solve this ?
        // this.existingTaskTypes(mappedTaskTypes);
        
        // ORG:
        // this.existingTaskTypes(this.tasksTypesOrderForUI.filter(x => _.includes(taskTypes, x))
        //     .map(ongoingTaskModel.mapTaskType));

        this.existingTaskTypes(this.tasksTypesOrderForUI.filter(x => _.includes(taskTypes, x))); // this is what compiles for now .. todo.. 
