...
class editKafkaEtlTask extends viewModelBase {
    ...
    static isApplyToAll = ongoingTaskKafkaEtlTransformationModel.isApplyToAll;
    ...
    activate(args: any) {
        ...
        if (args.taskId) {
            ...
            this.editedKafkaEtl().editedTransformationScriptSandbox(ongoingTaskKafkaEtlTransformationModel.empty(this.findNameForNewTransformation()));
            deferred.resolve();
        }
    ...
    saveEditedTransformation() {
        ...
        if (transformation.isNew()) {
            const newTransformationItem = new ongoingTaskKafkaEtlTransformationModel(transformation.toDto(), true, false); 
            ...
            const newItem = new ongoingTaskKafkaEtlTransformationModel(transformation.toDto(), false, transformation.resetScript());
    ...
    createCollectionNameAutoCompleter(usedCollections: KnockoutObservableArray<string>, collectionText: KnockoutObservable<string>) {
        return ko.pureComputed(() => {
            ...
            if (!_.includes(this.editedKafkaEtl().editedTransformationScriptSandbox().transformScriptCollections(), ongoingTaskKafkaEtlTransformationModel.applyToAllCollectionsText)) {
                result.unshift(ongoingTaskKafkaEtlTransformationModel.applyToAllCollectionsText);
            }
    ...
    removeTransformationScript(model: ongoingTaskKafkaEtlTransformationModel) {
