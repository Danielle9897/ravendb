    ...
    public static class SetupManager
    {
        ...
        private static async Task<X509Certificate2> CompleteAuthorizationAndGetCertificate(Action onValdiationSuccessful, SetupInfo setupInfo, Dictionary<string, Task<Challenge>> dictionary, AcmeClient acmeClient, (Dictionary<string, string> Challanges, byte[] Key) challengeResult, LetsEncryptCache cache, ServerStore serverStore, CancellationToken token)
        {
            ...
            try
            {
                ...
                csr.AddName($"CN={BuildHostName("a", setupInfo.Domain, setupInfo.RootDomain)}");

                foreach (var node in setupInfo.NodeSetupInfos)
                {
                    csr.SubjectAlternativeNames.Add(BuildHostName(node.Key, setupInfo.Domain, setupInfo.RootDomain));
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                var certBytes = pfxBuilder.Build(setupInfo.Domain.ToLower() + "." + setupInfo.RootDomain.ToLower() + " cert", "");
            ...
            try
            {
                    ...
                    cache.CertificateInstance.NotAfter > DateTime.Now.AddDays(3))
                {
                    if (string.Equals(cache.Domain, setupInfo.Domain, StringComparison.OrdinalIgnoreCase) 
                        && string.Equals(cache.RootDomain, setupInfo.RootDomain, StringComparison.OrdinalIgnoreCase))
                    {
                        var names = GetCertificateAlternativeNames(cache.CertificateInstance).ToArray();
                        var allExists = setupInfo.NodeSetupInfos.Keys.All(tag =>
                        {
                            var host = BuildHostName(tag, setupInfo.Domain, setupInfo.RootDomain);
                ...
                {
                    var host = BuildHostName(tag.Key, setupInfo.Domain, setupInfo.RootDomain);
